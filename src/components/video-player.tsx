    ...
    <div role="tablist" aria-label="Playback sources" className="flex flex-wrap gap-2">
      {sources.map((source) => (
        <button
          ...
          className={cn(
            "rounded-lg border px-4 py-2 text-sm font-medium transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-gold",
            source.id === activeId
...
>(function VideoPlayer({ sources, poster, title }, ref) {
  ...
  if (!sources.length || !active) {
    return (
      <div className="flex aspect-video w-full flex-col items-center justify-center gap-3 rounded-card border border-border bg-card text-center">
  ...
  return (
    <div className="space-y-3">
      <div data-player className="relative aspect-video w-full overflow-hidden rounded-card border border-border bg-black">
        {active.kind === "embed" ? (
          <iframe
            src={active.url}
      ...
      {sources.length > 1 && (
        <div>
          <p className="mb-2 text-xs font-medium uppercase tracking-wider text-muted">Servers</p>
