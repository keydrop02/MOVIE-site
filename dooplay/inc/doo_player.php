...
class DooPlayer{
		 ...
		 if($post_id && $play_nm){
            ...
            switch ($post_ty) {
                case 'movie':
	                ...
	                $superembedUrl = $this->checkForMovie( $postmeta );
                    break;
                case 'tv':
                    $postmeta = doo_postmeta_episodes($post_id);
	                $superembedUrl = $this->checkTv( $postmeta );
                    break;
            }
            ...
            // verify data
			if ( ! empty( $play_nm ) && $play_nm == 'superembed' ) {
		        $typ = 'superembed';
	        }
            if($typ){
                switch($typ){
     ...
     * @version 1.0
     */
    public static function viewer($post, $type, $players, $trailer, $size, $views, $ads = false, $image = false, $superembed_title = ""){
		// Set Ajax Player
        ...
        if($players OR $trailer){
            ...
            if(!empty($players) && is_array($players)){
                ...
                foreach($players as $play){
					...
					//superembed begins here
	                if($play['select'] == "superembed"){
		                $html .="<li id='player-option-{$num}' class='dooplay_player_option' data-type='{$type}' data-post='{$post}' data-nume='superembed'>";
		                $html .="<i class='fas fa-play-circle'></i>";
					$html .="<span class='title'>{$superembed_title}</span>";
		                $html .="<span class='server'>Multi source</span>";
	...
	public function checkTv( $post ) {
		...
		if ( ! empty( $post['temporada'] ) ) {
			$ids          = $post['ids'];
			$superembed = '/se_player.php?video_id='.$ids.'&tmdb=1&s='.$post['temporada'].'&e='.$post['episodio'] ;
		}
	...
	public function checkForMovie( $post ) {
		...
		return '/se_player.php?video_id='.$post['ids'];
