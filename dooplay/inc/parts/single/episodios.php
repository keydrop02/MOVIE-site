...
$superembed_title = $title_data['name']." season ".$title_data['season']." episode ".$title_data['episode'];
// End PHP
if ( ! empty( $player ) ) {
	$superembedPlayer = [
		count( $player ) + 1 =>
			[
				...
				'select' => 'superembed',
				'idioma' => '',
				'url'    => 'superembed.stream'
			]
	];

	$player = array_merge( $player, $superembedPlayer );
} else {
	$player = [
		0 =>
			[
...
<div id="single" class="dtsingle">
	...
	<div class="content <?php echo $sidebar; ?>">
        ...
        <?php DooPlayer::viewer($post->ID, 'tv', $player, false, $player_wht, $tviews, $player_ads, $dynamicbg, $superembed_title); ?>
        <!-- Episodes paginator -->
