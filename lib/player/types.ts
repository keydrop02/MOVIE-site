export type PlaybackKind = "hls" | "dash" | "mp4" | "embed" | "superembed";

/** Context passed to a provider that renders its own player UI. */
export interface SuperEmbedMeta {
  type: "movie" | "tv";
  tmdbId: number;
  season?: number;
  episode?: number;
...
export interface PlaybackSource {
  ...
  kind: PlaybackKind;
  url: string;
  /** Context for `kind === "superembed"` custom players. */
  meta?: SuperEmbedMeta;
